...
parser.add_argument("--rtrain", type=str,
                    default="/home/reul/Documents/Projects/ocropus_mptv/ocropus-rtrain",
                    help="path to ocropus-rtrain")
...
args.rtrain = os.path.expanduser(args.rtrain)
...
def run_single(params):
    ...
    subprocess.call([args.rtrain,
                     "-c", os.path.join(folds_dir, "*", "*.gt.txt"), args.whitelist,]
                    + [] if len(model) == 0 else ["--load", os.path.join(args.models_dir, model + ".pyrnn.gz"),]
                    + ["-o", output_model_dir + "/",
                     "--start", "0",
                     "-N", str(args.train_steps)] + lines)
